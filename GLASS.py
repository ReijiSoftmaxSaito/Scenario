...
def train_one_epoch(dataloader, model, epoch, config, device, **kwargs):
    ...
    for i_iter, data_item in enumerate(pbar, start=1):
        ...
        for step in range(model.step + 1):
            ...
            # Gradient Ascentを用いるため、異常特徴の勾配を取る
...
def eval_once(test_dataloader, model, args, broken, device):
    ...
    for anomaly, data, targets in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        # show_anomaly(data, targets, masks.unsqueeze(1), f"result/{args.result}/_{args.scenario}-exp_{args.category}-{broken}/{idx}.png", limit=[anoma_min, anoma_max])
        idx += 1
...
def evaluate(args):
    ...
    for broken in broken_type:
        ...
        # result_path = os.path.join("result", args.result, f"_{args.scenario}-exp_{args.category}-{broken}", "_result.txt")
        # with open(result_path, "w") as f:
