from model.GLASS.loss import FocalLoss
from collections import OrderedDict
from torchvision import transforms
from torch.utils.tensorboard import SummaryWriter
from model.GLASS.model import Discriminator, Projection, PatchMaker
...
import torch
import tqdm
from model.GLASS import common
import metrics
import cv2
import utils
import glob
...
class NoiseGenerator(torch.nn.Module):
    def __init__(self, noise, embed_dim, device):
        super(NoiseGenerator, self).__init__()
        self.noise = torch.nn.Parameter(torch.normal(0, noise, [8192, 1536])).to(device)
        # self.noise_proj = torch.nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, x):
        true_feat_proj = self.noise_proj(x)
        noise = self.noise * true_feat_proj
        return noise
...
class GLASS(torch.nn.Module):
    ...
    def load(
        ...
        self.noise_gen = NoiseGenerator(noise, self.target_embed_dimension, self.device)
        self.dsc_opt = torch.optim.AdamW(list(self.discriminator.parameters()) + list(self.noise_gen.parameters()), lr=lr * 2)
